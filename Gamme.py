...
class Texture:
    ...
    texture_names = [0,1,2,3]  #4 textures (BG - Player - Block - Help)
    ...
    def loadTextures(self):
        ...
        images.append(pygame.image.load("block.png"))
        images.append(pygame.image.load("conner.jpg")) 
        images.append(pygame.image.load("help.jpg"))  
        ...
        for i in range(len(images)):
            self.texture_setup(textures[i],  # binary images
                        ...
                        images[i].get_width(),
                        images[i].get_height())
    def draw(self): # for background
        glBindTexture(GL_TEXTURE_2D, self.texture_names[0])  
        glBegin(GL_QUADS)
        glTexCoord2f(0, 1)  
        glVertex2d(-9, 5)
        glTexCoord2f(1, 1)
...
class gameState:
    ...
    def updateState():
        if gameState.currentState == gameState.start:
            Text.drawString(" S P A C E   G A M E", 1500, 1850, 0.4, 3)
            Text.drawString("  P R E S S  __( P )__ T O  P L A Y ", 3250, 1650, 0.2, 3)
            Text.drawString(" P R E S S   __( E S C )__ T O  E X I T", 3150, 3450, 0.2, 3)
            ...
            if gameState.clickedP:
                ...
                gameState.clickedP = False # if you wanna to plat again 
            ...
            Text.drawString("   P R E S S __( P )__ T O  P L A Y  A G A I N ", 2750, 1650, 0.2, 3)
            Text.drawString("   P R E S S __( E S C )__ T O  E X I T ", 3050, 1350, 0.2, 3)
            
            if gameState.clickedP == True:
                gameState.currentState = gameState.reset
                gameState.clickedP = False

        elif gameState.currentState == gameState.reset: # every thing is deleted
            spawnBlocks.reset()
            spawnHelpers.reset()
            Player.reset()
            Score.resetCurrent()
...
class Block:
    ...
    def __init__(self,playerX, playerY, x, y, z, scaleFactor):
        ...
        self.y = y
        self.z = z
        self.scaleFactor = scaleFactor #volume
    ...
    def blockShape(self):
        ...
        for i in range(360):
                x = 0.5 * math.cos(i)
                y = 0.5  * math.sin(i)
                glTexCoord2f((x + 1) / 2, (y + 1) / 2) #CHAT
                glVertex2f(x, y)  # Vertex on the circle
        glEnd()

    def updateMovement(self):
        if self.scaleFactor <= 1 : # split speed decrease
            self.blockSpeed = .01
        if self.x <= -4 :
            self.leftCollided =True
        elif self.x >= 4 :
            self.leftCollided =False
        elif self.y >= 2 or (self.scaleFactor >= 2 and self.y >= 0.5): # abosettooooooo
    ...
    def draw(self):
        if self.visible:
            ...
            if Player.visible:
                if not self.bulletCollided: # bullet collision in block 
                    self.detectBulletsCollision()
                    self.splitBlock()
                if Player.collided: #if 2 blocks coliided at the same time
                    now = time.time()
    ...
    def killAnimation(self): # for kill
        if self.kill:
...
class spawnBlocks: # view block at the screen
    blocksNumber = 15
    spawned = []
    spawnTime = time.time()
    def init():
        ...
        for i in range(spawnBlocks.blocksNumber//3):
            block = Block(Player.xPos, Player.yPos,random.randrange(-4, 4),random.randrange(1,2) ,Player.zPos, random.randrange(1, 3,1) )
            spawnBlocks.spawned.append(block)
            
        for i in range(spawnBlocks.blocksNumber//3):
            block = Block(Player.xPos, Player.yPos,random.randrange(-4, 4),random.randrange(1,2) ,Player.zPos, random.randrange(1, 5,1) )
            spawnBlocks.spawned.append(block)
...
class Bullet:
    ...
    def updateMovement(self,step = 0.05):
        ...
        if self.bulletPosition[1]>= 2.5: #longest path
            self.visible = False
...
class Display:
    ...
    def perspectiveProjection(): # as camera (playing)
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluPerspective(50,(Display.width/Display.height), 1, 100) # f0vy & aspect & z near & Zfar
        glMatrixMode(GL_MODELVIEW)

    def orthographicProjection(): # as isometric (start & gameove)
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluOrtho2D(0, Display.width, 0, Display.height) # left right top bottom
        glMatrixMode(GL_MODELVIEW)
...
class Text:
    def drawString(text, xPos, yPos, scaleFactor, lineWidth = 2, fontID = 1):
        ...
        glColor(1, 1, 1, 0)  # Set text color to white
...
class Player:
    ...
    def newBullet():
        newBullet = Bullet(Player.xPos , Player.yPos+0.2, Player.zPos)
